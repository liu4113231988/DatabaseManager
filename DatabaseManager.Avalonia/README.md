...
- **对象浏览**：对象树支持数据库、Schema、表、视图、过程、函数、序列及列/索引/键/约束/触发器多级懒加载；树内就地过滤（输入即时隐藏不匹配节点，清空恢复）与深度搜索定位、右键菜单（Generate SQL、新建/删除/重命名对象、过滤模板、比较迁移入口，菜单图标按动作统一）、元数据搜索窗口与菜单扩展点。
...
当前版本已具备日常数据库浏览、查询、编辑、建表、跨库处理和运维监控的主链路。功能决策、优先级与明确不做清单见根目录 [Roadmap.md](../Roadmap.md)，最新批次评估与实施记录见 [docs/p3-evaluation-20260917.md](./docs/p3-evaluation-20260917.md)。KingbaseES 的支持边界与验证状态见 [docs/kingbasees-support-plan.md](./docs/kingbasees-support-plan.md)。
