...
public static class DbAdminGuidance
{
    public static string GetSessionPermissionHint(DatabaseType databaseType) => databaseType switch
    {
        ...
        DatabaseType.KingbaseES => "优先使用 sys_stat_activity、sys_locks、sys_terminate_backend（需 sys_signal_backend 权限）；若实例未暴露 sys_* 视图/函数，会自动回退到 pg_stat_activity、pg_locks、pg_terminate_backend（需 pg_signal_backend 权限）。终止超级用户会话仅限超级用户。",
