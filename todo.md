...
- [x] **图表/仪表盘增强**：保存可配置取样上限（1-1000），无聚合与分组聚合均应用该限制；已保存图表可编辑 SQL、重新取数并保存更新。
- [x] **查询剖析增强**：服务端计划扩展至 SQL Server（STATISTICS XML）与 Oracle（DBMS_XPLAN `ALLSTATS LAST`）；持续保持仅单条只读 SELECT 的安全限制。
