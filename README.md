...
- **连接与对象管理**：连接的新建、编辑、测试、刷新与删除；对象树按数据库、Schema、对象类型和对象明细分级加载；对象树内置元数据搜索，双击定位到树中；大型目录支持懒分页加载。
- **SQL 开发与执行**：多标签查询、语法高亮、关键字/数据库对象/字段智能提示、执行结果与消息输出、Commit / Rollback / 自动提交切换；支持查询历史、脚本库、代码片段、SQL 格式化和执行计划分析；单次查询超时与取消、危险 DDL/DML 二次确认。
- **数据与结构操作**：分页查看和编辑表/视图数据（增删改、脏列追踪、事务保存）；表、列、主键、索引、外键、约束的设计、DDL 预览与保存。
- **数据工程与运维**：数据库转换、结构对比、数据对比、Schema 映射与列映射预览、依赖分析、诊断、优化、统计、索引碎片分析、数据库备份与恢复。
- **导入导出**：CSV / Excel / SQL / JSON / XML 等多种格式的数据导入导出，支持列映射预览与校验。
- **交付辅助**：代码生成（C# / Java）、列结构文档生成、图片和 JSON 内容查看。

### 体验与扩展性（P2）

- **任务中心**：运行中/完成/失败/取消的任务列表，支持任务取消、日志查看与跨会话历史；转换、导入导出、备份、统计等已接入。
- **工作区持久化**：窗口大小/位置、对象树面板宽度自动记忆并跨会话恢复；未保存的 SQL 草稿随查询页会话恢复。
- **主题与可访问性**：亮色 / 深色 / 高对比三套主题，状态栏支持外观切换与 90–125% 字体缩放；工具栏按钮补齐无障碍属性。
- **扩展机制**：对象树右键菜单支持贡献者扩展点（`IObjectTreeMenuContributor`）。
